...
1. Download `owl-sidecar-0.2.0-windows-x64.zip`.
2. Extract the zip.
...
```sh
sudo dpkg -i owl-sidecar-0.2.0-linux-amd64.deb    # Intel and AMD
sudo dpkg -i owl-sidecar-0.2.0-linux-arm64.deb    # ARM
```
...
1. Download `owl-sidecar-0.2.0-linux-x86_64.tar.gz` (or `-linux-aarch64` on ARM).
2. Extract it: `tar -xzf owl-sidecar-0.2.0-linux-x86_64.tar.gz`
